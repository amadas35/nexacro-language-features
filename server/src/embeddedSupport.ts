	...
	while (token !== TokenType.EOS) {
		switch (token) {
			...
			case TokenType.Script:
				if (languageIdFromType?.toLowerCase() === 'xscript') {
					const offset = scanner.getTokenOffset();
					const src = scanner.getTokenText();
					const matched = src.match(/(?<=^<!\[CDATA\[)([\s\S]*)(?=]]>$)/gm);
					if (matched) {
						const prefixchars = ('<![CDATA[').length;
						const sufixchars = (']]>').length;

						regions.push({ 
							languageId: languageIdFromType, 
							start: offset + prefixchars, 
							end: scanner.getTokenEnd() - sufixchars 
						});
					}
				}
				else {
			...
			case TokenType.AttributeValue:
				if (lastAttributeName === 'src' && lastTagName.toLowerCase() === 'script') {
					...
					} else if (/["']xscript[0-9]\.[0-9]["']/.test(scanner.getTokenText())) {
						languageIdFromType = 'xscript';
					} else {
...
function getEmbeddedDocument(document: TextDocument, contents: EmbeddedRegion[], languageId: string, ignoreAttributeValues: boolean): TextDocument {
	...
	for (const c of contents) {
		if (c.languageId === languageId && !c.attributeValue) {
