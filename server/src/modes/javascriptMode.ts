...
function getLanguageServiceHost(scriptKind: ts.ScriptKind) {
	const compilerOptions: ts.CompilerOptions = { 
		...
		checkJs: true, 
		lib: ['lib.es6.d.ts'], 
		types: ["nexacro", "nexacro-compbase", "nexacro-comcomp", "nexacro-grid", "nexacro-listview"],
		target: ts.ScriptTarget.Latest, 
		moduleResolution: ts.ModuleResolutionKind.NodeNext, 
		experimentalDecorators: false 
	};

	let currentTextDocument = TextDocument.create('init', 'xscript', 1, '');
...
export function getJavaScriptMode(documentRegions: LanguageModelCache<HTMLDocumentRegions>, languageId: 'xscript' | 'javascript' | 'typescript', workspace: Workspace): LanguageMode {
	...
	const globalSettings: Settings = { html: true, css: false, javascript: true };
